...
2. 使用一般訓練迴圈**預熱**觀察器。
3. **凍結**啟用值範圍與 SiMa 相容的權重比例。
4. 使用鎖定的比例繼續訓練，以**恢復**準確度。
5. **完成**用於推論的模型。
...
此命令會下載 wheel，並安裝或更新供 Codex 與 Claude 使用的 QAT 編碼代理技能。
...
若要繼續訓練，請使用相同的範例輸入與批次合約重新建立並準備相同模型，再載入
儲存的狀態：
...
準備程序預設會將張量的第一個維度保持為動態批次。因此，同一個準備後的圖可以使用
