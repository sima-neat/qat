...
Each entry records PyTorch capture, quantization behavior, standard ONNX export,
and test obligations. Deployment support is intentionally outside this manifest.
"""
...
OPERATOR_MANIFEST_VERSION = "3.0.0"
ONNX_OPSET = 17
...
def _entry(
    ...
    *,
    ...
    return OperatorManifestEntry(
        ...
        expected_onnx_operators=onnx or (onnx_operator,),
...
OPERATOR_MANIFEST = (
    _entry(
        ...
        dtypes=W8A8,
        constraints=("static floating weights",),
        annotator="conv",
    ...
    _entry(
        ...
        dtypes=("floating tensors remain outside the QAT annotation contract",),
        constraints=("ConvTranspose weight annotation is not implemented",),
        behavior=QuantizationBehavior.PASSTHROUGH,
        positive=(),
        negative=("ConvTranspose remains trainable and unannotated",),
    ),
    _entry(
        "linear",
        "Gemm",
        ...
        dtypes=W8A8,
        constraints=("static floating weight", "floating bias when present"),
        annotator="linear",
    ...
    _entry(
        ...
        constraints=("exactly two floating operands", "static equation"),
    ...
    _entry(
        ...
        constraints=("same shape, scalar, or PyTorch broadcasting",),
        annotator="add",
    ...
    _entry("logarithm", "Log", 13, functions=("torch.log",), aten=("aten.log.default",), constraints=("positive input domain after input QDQ; invalid activation qparams reject freeze",), annotator="sima_unary_int8", cases=("log",), negative=("exercise non-positive-domain diagnostics",)),
    _entry("negate", "Neg", 13, functions=("torch.neg", "operator.neg"), aten=("aten.neg.default",), annotator="sima_unary_int8", cases=("neg",)),
    _entry("reciprocal", "Reciprocal", 13, functions=("torch.reciprocal",), aten=("aten.reciprocal.default",), constraints=("nonzero input after input QDQ; invalid activation qparams reject freeze",), annotator="sima_unary_int8", cases=("reciprocal",)),
    ...
    _entry(
        ...
        constraints=("constant scalar exponent",),
        annotator="sima_pow",
        cases=("pow",),
        negative=("dynamic tensor exponents are not annotated by this pattern",),
    ),
    _entry(
        "prelu",
        "PRelu",
        ...
        behavior=QuantizationBehavior.PASSTHROUGH,
        dtypes=("floating tensors remain outside the QAT contract",),
        constraints=("PReLU is intentionally left unannotated",),
        positive=(),
        negative=("PReLU receives no QAT annotation or fake-quant boundary",),
    ),
    _entry(
        "relu",
        "Relu",
    ...
    _entry(
        ...
        negative=("approximate='tanh' remains trainable and unannotated",),
    ),
    _entry(
        "softmax",
        "Softmax",
        ...
        constraints=("static logical axis",),
        annotator="sima_softmax",
        cases=("softmax",),
        onnx_cases=("softmax",),
    ),
    _entry("log_softmax", "LogSoftmax", 13, modules=("torch.nn.LogSoftmax",), functions=("torch.log_softmax", "torch.nn.functional.log_softmax"), aten=("aten.log_softmax.int",), constraints=("static logical axis",), annotator="sima_unary_int8", cases=("log_softmax",)),
    _entry("layer_norm", "LayerNormalization", 17, modules=("torch.nn.LayerNorm",), functions=("torch.nn.functional.layer_norm",), aten=("aten.layer_norm.default",), constraints=("floating scale and bias remain unquantized",), annotator="sima_layer_norm", cases=("layer_norm",), onnx_cases=("layer_norm",)),
    _entry("instance_norm", "InstanceNormalization", 6, modules=("torch.nn.InstanceNorm2d",), functions=("torch.nn.functional.instance_norm",), aten=("aten.instance_norm.default",), constraints=("rank 4 or 5", "input statistics (track_running_stats=False)"), annotator="sima_unary_int8", cases=("instance_norm",)),
    _entry(
        "local_response_norm",
        "LRN",
        13,
    ...
    _entry(
        ...
        behavior=QuantizationBehavior.DECOMPOSED,
        constraints=("variance must be nonzero",),
        positive=(),
        negative=("composite topology and zero-variance behavior require tests",),
    ),
    _entry("average_pool", "AveragePool", 11, modules=("torch.nn.AdaptiveAvgPool2d",), functions=("torch.nn.functional.adaptive_avg_pool2d",), aten=("aten.adaptive_avg_pool2d.default",), annotator="adaptive_avg_pool2d", cases=("adaptive_avg_pool2d",), onnx_cases=("adaptive_avg_pool2d",)),
    ...
    _entry("max_pool", "MaxPool", 12, modules=("torch.nn.MaxPool2d",), functions=("torch.nn.functional.max_pool2d",), aten=("aten.max_pool2d.default",), behavior=QuantizationBehavior.PROPAGATION, dtypes=GRID, constraints=("value output without returned indices",), annotator="propagate_annotation", cases=("max_pool2d",), onnx_cases=("max_pool2d",)),
    ...
    _entry("reduce_mean", "ReduceMean", 13, functions=("torch.mean",), aten=("aten.mean.dim",), constraints=("static reduction axes",), annotator="sima_reduction", cases=("reduce_mean",)),
    _entry("reduce_sum", "ReduceSum", 13, functions=("torch.sum",), aten=("aten.sum.dim_IntList",), constraints=("static reduction axes",), annotator="sima_reduction", cases=("reduce_sum",)),
    _entry("reduce_max", "ReduceMax", 13, functions=("torch.amax",), aten=("aten.amax.default",), annotator="sima_reduction", cases=("reduce_max",)),
    _entry("reduce_l1", "ReduceL1", 17, functions=("torch.linalg.vector_norm(ord=1)",), aten=("aten.linalg_vector_norm.default",), constraints=("static axes",), annotator="sima_reduction", cases=("reduce_l1",)),
    _entry("reduce_log_sum_exp", "ReduceLogSumExp", 17, functions=("torch.logsumexp",), aten=("aten.logsumexp.default",), constraints=("static axes",), annotator="sima_reduction", cases=("reduce_logsumexp",)),
    ...
    _entry("variadic_mean", "Mean", 13, status=SupportStatus.DEFERRED, functions=("stack/add followed by divide",), aten=("no direct PyTorch ATen equivalent",), behavior=QuantizationBehavior.DECOMPOSED, constraints=("same-shaped inputs",), positive=(), negative=("add an exported composite contract",)),
    _entry("variadic_sum", "Sum", 13, status=SupportStatus.DEFERRED, functions=("sum of a tensor sequence",), aten=("chain of aten.add.Tensor",), behavior=QuantizationBehavior.DECOMPOSED, constraints=("PyTorch broadcasting rules",), positive=(), negative=("add a variadic-input topology test",)),
    _entry("concat", "Concat", 13, functions=("torch.cat",), aten=("aten.cat.default",), constraints=("floating tensor inputs",), annotator="sima_cat", cases=("cat",), onnx_cases=("cat",)),
    _entry("expand", "Expand", 13, functions=("torch.Tensor.expand",), aten=("aten.expand.default",), behavior=QuantizationBehavior.PROPAGATION, dtypes=GRID, constraints=("static broadcast-compatible shape",), annotator="sima_grid_preserving", cases=("expand",)),
    ...
    _entry("flatten", "Flatten", 13, functions=("torch.flatten",), aten=("aten.flatten.using_ints",), behavior=QuantizationBehavior.PROPAGATION, dtypes=GRID, constraints=("static start and end dimensions",), annotator="sima_grid_preserving", cases=("flatten",), negative=("retain explicit opset-17 export coverage",)),
    _entry("transpose", "Transpose", 13, functions=("torch.transpose", "torch.permute"), aten=("aten.transpose.int", "aten.permute.default"), behavior=QuantizationBehavior.PROPAGATION, dtypes=GRID, constraints=("static permutation",), annotator="sima_grid_preserving", cases=("transpose",)),
    ...
    _entry("padding", "Pad", 13, functions=("torch.nn.functional.pad",), aten=("aten.pad.default",), behavior=QuantizationBehavior.PROPAGATION, dtypes=GRID, constraints=("captured pad consumes a floating tensor",), annotator="sima_grid_preserving", cases=("pad",)),
    ...
    _entry("tile", "Tile", 13, functions=("torch.tile", "torch.Tensor.repeat"), aten=("aten.tile.default", "aten.repeat.default"), behavior=QuantizationBehavior.PROPAGATION, dtypes=GRID, constraints=("static repeats",), annotator="sima_grid_preserving", cases=("tile",)),
    _entry("resize", "Resize", 13, functions=("torch.nn.functional.interpolate",), aten=("aten.upsample_nearest2d.vec", "aten.upsample_bilinear2d.vec"), constraints=("captured nearest-neighbor or bilinear 2D form",), annotator="sima_unary_int8", cases=("resize_nearest", "resize_bilinear"), negative=("other captured interpolation operators remain unannotated",)),
    _entry("argmax", "ArgMax", 13, functions=("torch.argmax",), aten=("aten.argmax.default",), behavior=QuantizationBehavior.TYPE_PRESERVING, dtypes=("signed INT8 input", "torch.int64 index output"), constraints=("static axis",), annotator="sima_mixed_output", cases=("argmax",), negative=("index output must never receive fake quantization",)),
    _entry("topk", "TopK", 11, functions=("torch.topk",), aten=("aten.topk.default", "operator.getitem(value/index)"), behavior=QuantizationBehavior.MIXED_OUTPUT, dtypes=("signed INT8 values", "torch.int64 indices"), constraints=("constant K", "rank >= 2"), annotator="sima_mixed_output", cases=("topk_values",), negative=("indices remain unquantized",)),
    _entry(
        "batch_norm_training",
        "BatchNormalization",
        15,
        ...
        constraints=("training statistics are frozen before finalization",),
        annotator="sima_batchnorm",
        cases=("batchnorm", "conv_bn", "conv_bn_relu", "conv_bn_hardtanh"),
        negative=("finalization preserves inference behavior",),
    ),
    _entry("embedding_gather", "Gather", 13, status=SupportStatus.DEFERRED, modules=("torch.nn.Embedding",), functions=("torch.nn.functional.embedding",), aten=("aten.embedding.default",), behavior=QuantizationBehavior.PASSTHROUGH, dtypes=("floating table weights", "integer indices"), constraints=("integer indices remain integer"), positive=(), negative=("Embedding remains trainable and unannotated",)),
    _entry("grid_sample", "GridSample", 16, status=SupportStatus.DEFERRED, functions=("torch.nn.functional.grid_sample",), aten=("aten.grid_sampler.default", "aten.grid_sampler_2d.default"), behavior=QuantizationBehavior.PASSTHROUGH, dtypes=("floating input and grid tensors",), constraints=("GridSample is not annotated",), positive=(), negative=("GridSample remains trainable and unannotated",)),
    _entry("reduce_min", "ReduceMin", 13, status=SupportStatus.DEFERRED, functions=("torch.amin",), aten=("aten.amin.default",), behavior=QuantizationBehavior.PASSTHROUGH, dtypes=("floating tensors remain outside the QAT annotation contract",), constraints=("ReduceMin is not annotated",), positive=(), negative=("ReduceMin remains trainable and unannotated",)),
    _entry("cumulative_sum", "CumSum", 14, status=SupportStatus.DEFERRED, functions=("torch.cumsum",), aten=("aten.cumsum.default",), behavior=QuantizationBehavior.PASSTHROUGH, dtypes=("floating tensors remain outside the QAT annotation contract",), constraints=("CumSum is not annotated",), positive=(), negative=("CumSum remains trainable and unannotated",)),
)
